...
// keep an ordinary basename (Coco/R Parser.h/Scanner.h), kernel UAPI headers copied
// into a tree (Linux-syscall-note), and files explicitly "copied from" elsewhere.
// Unlike hasVendorLicenseHeader this is NOT guarded by the dominant-banner ratio:
// these markers are per-file truth, not a project-wide license style.
//
...
// bpftrace"), "imported from" (functions.h: "imported from pre-compiled BPF
// programs"), "do not edit" ("do not edit lightly"), and "copied from" — a corpus
// check (#131) found it matches 82/91 times OUTSIDE any vendor signal, with a
// confirmed own-code false positive on libjxl; its only unique coverage was one
// file (bpf_assembler.h "copied from bcc") that no other layer catches, not worth
// the prose surface. Widen only with a corpus measurement of what a marker drops.
inline bool hasGeneratedHeader(std::string_view headerBytes)
{
  const std::string head = toLowerAscii(headerBytes.substr(0, std::min<std::size_t>(headerBytes.size(), 2000)));
  static constexpr std::array<std::string_view, 3> kGeneratedHeaderMarkers = {
      "@generated",         // machine-generated tag (the '@' anchors it as a tag, not prose)
  ...
  for (std::string_view m : kGeneratedHeaderMarkers)
  {
    if (head.find(m) != std::string::npos)
    {
      return true;
