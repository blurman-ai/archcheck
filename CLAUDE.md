...
## Самопроверка выводов (обязательно)

Повторяющийся паттерн, который надо сломать: получен результат прогона → делается вывод «работает правильно» → результат показан пользователю → пользователь говорит «подозрительно, перепроверь» → перепроверка подтверждает, что вывод был необоснован. Чтобы не пинать этот мяч туда-сюда:

- **Роль скептика — твоя, до показа результата.** Прежде чем заключить «результат правильный / сканер работает», сам задай вопросы, которые задал бы пользователь: правдоподобны ли числа? не слишком ли гладко? что бы выглядело так же при сломанной логике? Проверь 2–3 конкретных случая руками (вход → ожидаемый выход), а не только итоговую сводку.
- **Сам факт, что прогон завершился и выдал числа, — не доказательство корректности.** Вывод «правильно» должен опираться на независимую перекрёстную проверку, а не на внутренний шаблон «прогон прошёл → ок».
- **Поймал себя на необоснованном выводе — перепроверь до отправки**, а не после возражения пользователя.
- **Осталась неуверенность — донеси сомнения явно**: что именно не проверено и что выглядит подозрительно.
- **Не закрывать задачу, пока нет полной уверенности, что результаты адекватны.** Сомнения остались → задача открыта, сомнения — пользователю.

## Журнал пути — JOURNEY.md (обязательно вести)

[docs/JOURNEY.md](docs/JOURNEY.md) — живой human-in-the-loop журнал **находок, тупиков и разворотов**
(тело будущей статьи). **Правило: каждую ключевую находку и значимый эпизод дописывать в JOURNEY по
горячим следам** — не откладывать, не «вспомним потом по git log» (git log этого не хранит). Дописывай,
когда происходит любое из:
- **ключевая находка / результат** — вывод корпус-прогона, подтверждённый/опровергнутый сигнал, число
  с вердиктом (что доказали или опровергли, и чем убит сигнал);
- нетривиальный успех (нашли причину, разворот метода, прорыв);
- **тупик / ошибочный вывод**, который поймали (особенно если поймал автор репликой);
- момент, где steering автора развернул работу.

Формат: дата + контекст + ход агента/гипотеза + **дословная** реплика автора (если был разворот;
для находки-результата опционально) + разбор + урок/вывод + коммит. Писать честно, в т.ч. (и особенно)
про ошибки агента и про null-результаты — в этом ценность. Не путать с памятью: memory = атомарный
recall-факт; JOURNEY = нарратив. Крупные доказательные результаты живут ещё и в
[docs/research/central_finding.md](docs/research/central_finding.md); в JOURNEY они идут как веха пути.
