...
OUT=~/projects/cpparch/experiments/ai_repo_run/discipline.tsv
LOG=~/projects/cpparch/experiments/ai_repo_run/discipline.log
: > "$LOG"
...
# 100+ корпус
for d in ~/oss/_aidev_run/*/; do
  [ -d "$d/.git" ] || continue
  run_one "100+" "$d"
done
# низко-звёздные бакеты
for bdir in ~/oss/_aidev_lowstar/*/; do
  b=$(basename "$bdir")
  for d in "$bdir"*/; do
    [ -d "$d/.git" ] || continue
    run_one "$b" "$d"
