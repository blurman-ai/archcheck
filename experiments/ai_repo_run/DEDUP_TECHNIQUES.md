...
Исследование эволюции архитектуры: для проектов, у которых доля дублированного кода
(`dup_ratio` из `line_duplication`) снизилась со временем, по git-истории установлено,
**какими инженерными приёмами** разработчики убирали копипаст.

Источник дат/окон: `experiments/ai_repo_run/runs_history/all/<repo>.tsv`,
сводка: `experiments/ai_repo_run/runs_history/drift_summary_v2.tsv`.
Каждый приём подтверждён через `git show` реального коммита в `/home/localadm/oss/_aidev_run/<repo>`.
