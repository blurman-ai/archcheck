...
> include-графе) на корпусе AIDev (`~/oss/_aidev_run/`). Здесь для
...
Перепроверка:
```bash
d=~/oss/_aidev_run/acts; AC=.../build/debug/src/archcheck
...
#056). Перепроверка:
```bash
git -C ~/oss/_aidev_run/OptiScaler show -s --format=%B cc5d695e
/tmp/partial_dup_build/partial_duplication --diff cc5d695e --repo ~/oss/_aidev_run/OptiScaler
```
...
```bash
d=~/oss/_aidev_run/opentelemetry-cpp
...
```bash
d=~/oss/_aidev_run/firmware; AC=.../build/debug/src/archcheck
...
**неясно**. Перепроверка:
```bash
d=~/oss/_aidev_run/FastLED; AC=.../build/debug/src/archcheck
