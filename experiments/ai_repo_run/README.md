...
## 1. Где живут клоны (корпус курирован — см. ниже)

```
/home/localadm/oss/_aidev_run/<repo-basename>/
```
...
> **Корпус курирован (2026-05-31).** Клоны, на которых archcheck ничего не нашёл
> или нашёл **только ложное** (вендоринг / fork-in-repo / data-blob /
> мета-агрегатор / codegen / header-only-фантом), удалены с диска — полный список
> и причины в [REMOVED.md](REMOVED.md) (**116 реп, ~6.0 ГБ**: high-star 29,
> low-star 87; диск `/home/localadm/oss` 26→20 ГБ). Критерий и воспроизводимость:
> `classify_remove.py` (классификация по `runs_history/*.tsv`) → `do_remove.py`
> (`--apply`). Сами TSV-сводки и серии НЕ тронуты — исследование воспроизводимо.
> Удалённую репу можно вернуть `git clone` по её URL.
...
```bash
cmake -S /home/localadm/projects/cpparch/experiments/line_duplication \
...
```bash
cd /home/localadm/projects/cpparch/experiments/ai_repo_run
...
- `CORPUS_DIR` — корень клонов (default `/home/localadm/oss/_aidev_run`).
