...
## 2026-06-12 — Проверили на сотне чужих проектов: умеет ли инструмент замечать рост сложности

**Версия archcheck:** <!-- TODO: уточнить commit -->

### Прогон 18 — 100 открытых C++ проектов (предупреждение «функция стала сложнее»)
- **Что проверяли:** правило, которое предупреждает, когда какая-то функция в
  очередном изменении становится заметно запутаннее, чем была. Прогнали его на
  сотне чужих проектов.
- **Зачем простыми словами:** прежде чем показывать такое предупреждение людям,
  надо убедиться, что оно срабатывает по делу, а не кричит попусту.
- **Итог:** все случаи, где инструмент ошибался, разобрали по одному, нашли
  причины и поправили (подробности и точные цифры — задача #109). После правок
  правило признано готовым к работе.
- **Главный вывод:** инструмент честно отличает «функция действительно
  усложнилась» от «просто добавили много однотипного, но простого кода».

---

## 2026-06-12 — Большой прогон: умеет ли инструмент замечать, что части проекта начали срастаться

**Версия archcheck:** <!-- TODO: уточнить commit -->

### Прогон 19 — 479 открытых проектов (новое правило о связях между частями проекта)
- **Что проверяли:** новое правило, которое замечает момент, когда две прежде
  независимые части проекта впервые начинают зависеть друг от друга. Это частый
  ранний признак того, что аккуратная структура проекта начинает «расползаться».
- **Масштаб:** 479 проектов, около 417 таких событий в их истории изменений.
- **Итог:** самый строгий вариант правила (когда новая связь замыкает круг
  зависимостей) оказывался верным примерно в 9 случаях из 10 — этого достаточно,
  чтобы применять его как блокирующую проверку. Более мягкие варианты оставлены
  как необязательные подсказки.
- **Главный вывод:** инструмент способен заранее показать момент, когда проект
  начинает терять чистоту структуры, и при этом редко поднимает ложную тревогу.
