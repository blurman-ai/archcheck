...
Дальше в job вызывается просто `archcheck …` (в `PATH`). Asset собран на
`ubuntu-24.04` (gcc-13, `-static-libstdc++ -static-libgcc`) — нужен свежий glibc;
для старых baseline берите fallback ниже. Платформа пока одна — Linux x86_64.
