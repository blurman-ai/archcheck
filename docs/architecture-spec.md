...
Open source CLI для проверки архитектурных инвариантов в C++ проектах. **Ядро — модульные границы и циклы зависимостей в CI:** пользователь описывает в YAML, какие модули какие могут импортировать, archcheck строит граф зависимостей и ломает сборку при нарушении. Нарушения репортятся с `file:line` (column — будущий semantic-backend), exit code — non-zero. Для legacy-проектов — `--baseline`: фриз текущих нарушений, новые ломают CI.
...
Второй, независимый замер того же эффекта — отчёт GitClear «AI Copilot Code Quality» (2024, обновлён 2025). На корпусе ~211 млн осмысленных изменённых строк за пять лет (коммерческие репы + крупный OSS вроде React и Chrome) зафиксировано падение числа перемещённых строк на ~40% и рост дублирования: 2024 стал первым годом, когда скопированных строк оказалось больше, чем перемещённых. Перемещение строк — прокси рефакторинга; его спад означает, что код перестаёт консолидироваться в переиспользуемые модули и растёт копипастой. EURECOM меряет structural assertion pass rate, GitClear — операции над строками в реальных коммитах; методы разные, вывод один — AI-код дрейфует структурно, а не только функционально. Оговорка: GitClear — vendor research (продают инструмент против ровно этой проблемы), не peer-reviewed, и причинность «больше AI → хуже метрики» у них оспаривают (Google DORA 2024 отчитался о небольшом росте качества). Поэтому здесь — как подтверждающий замер, а не как доказательство.
...
- **God-component (fan-out)** — зеркало god-header по out-degree: компонент, который сам зависит от слишком многих. Перегруз *выхода* (переусложнённый потребитель), тогда как god-header — перегруз *входа* (бутылочное горло). Тот же авторитет (Lakos, «minimize fan-out»). Планируется как дефолтное правило `Lakos.GodComponentFanOut` (#057); порог калибруется на OSS-корпусе, у fan-out распределение иное, чем у fan-in.
...
Дальше в документе этот процесс — когда AI-агент читает артефакты репозитория,
строит архитектурную гипотезу и превращает её в проверяемые правила —
называется **AI-assisted rule synthesis**.

**Operational shape (концептуально, формальный contract — см. задачу #010):**
synthesis это **отдельный shell-флоу**, который пользователь вызывает по своей
инициативе (`archcheck synthesize > .archcheck.yml.draft`), а не магия, которая
случается внутри обычного запуска `archcheck`. Сам archcheck **никаких LLM
не вызывает**: synthesis — это либо локальные эвристики (path-structure /
common headers / facade detection), либо отдельный wrapper-агент (Claude
Code / Cursor / ваш промпт), читающий тот же репозиторий и выдающий YAML.
Это сохраняет приватность кода (не уходит наружу без явного действия) и
делает archcheck-ядро детерминированным.
...
**Snapshot-анализ (intrinsic + user-declared rules на текущем состоянии репозитория) остаётся ядром продукта** — на нём держится первый запуск без конфига, дог-фудинг на топ-N OSS проектов из roadmap v0.5, и весь объём пользовательских архитектурных контрактов. **DRIFT — дополнительный слой** для сценария, когда есть baseline графа и есть смысл сравнивать «до» и «после». Класс правил ловит не «плохой код вообще», а **структурное ухудшение относительно baseline**.

DRIFT парно работает с `repo-inferred rules`: synthesis формализует архитектурную гипотезу из репозитория, DRIFT затем стережёт, чтобы эта гипотеза не деградировала. Вместе они образуют рабочий контур **AI-assisted rule synthesis + drift regression** — два полу-цикла одной задачи (инференс контракта ↔ регрессия от контракта), оба полезных в эпоху AI-кодинга.

#### Соотношение с обычным baseline
...
Полная спецификация формата `.archcheck.yml` v1 — [`docs/config_format.md`](config_format.md). Там зафиксированы три типа правил (`layers`, `independence`, `forbidden`), их семантика, четыре reference-примера (tiny / layered / legacy / mixed), список того, что в scope phase 1, и SemVer-контракт схемы.

Минимальный осмотрительный пример для контекста:
...
Расширения уровня `defaults`, `thresholds`, `baseline`, `ignore`, pattern-правил — phase 2, не текущий контракт. Подробности и rationale — в `docs/config_format.md`.
...
- Точная версия SF.7 (через AST вместо text-scan).
...
6. **AI-assisted rule synthesis: корректность и приватность (v0.3+).** Три риска одного процесса:
   - **False architectural hypothesis.** Агент читает `src/utils/` и решает что это entry point слой, или объявляет `domain` зависимым от `infrastructure` потому что увидел один transitive include. Митигация: synthesis-output всегда стартует в `report-only` / `warning`, требует явного `confirm` пользователя; output помечается комментариями `# synthesized: <hypothesis>` чтобы было видно происхождение каждого правила.
   - **Privacy.** Synthesis по определению означает «что-то читает твой код». Митигация: archcheck-ядро не вызывает LLM само (см. §«AI-assisted rule synthesis» / operational shape) — это отдельный shell-флоу, пользователь сам решает, какому агенту/API доверить исходники. По умолчанию синтез — локальные эвристики, без сети.
   - **Volatility.** Тот же репо + тот же агент → разные synthesis-результаты от запуска к запуску (из-за non-determinism LLM). Митигация: synthesis выдаёт diff к предыдущему `.archcheck.yml.draft`, не переписывает целиком; финальный конфиг — всегда явное действие пользователя.
...
- GitClear. *AI Copilot Code Quality.* 2024 (обновлён 2025). PDF.
