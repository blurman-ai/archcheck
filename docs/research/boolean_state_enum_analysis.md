...
- **Файл:** [bitstream.h](file://~/oss/AetherSDR/third_party/libmodem_core/bitstream.h)
...
- **Файл:** [ivgl.h](file://~/oss/AcademySoftwareFoundation_OpenImageIO/src/iv/ivgl.h)
...
- **Файл:** [ReactorNet.h](file://~/oss/Cantera_cantera/include/cantera/zeroD/ReactorNet.h)
...
- **Файл:** [CoolPropFluid.h](file://~/oss/CoolProp_CoolProp/include/CoolProp/CoolPropFluid.h)
...
- **Файл:** [vmEntry.h](file://~/oss/DataDog_java-profiler/ddprof-lib/src/main/cpp/vmEntry.h)
...
- **Файл:** [ImageBackingStore.h](file://~/oss/BlueSCSI_BlueSCSI-v2/src/ImageBackingStore.h)
...
- **Файл:** [BedrockCommand.h](file://~/oss/Expensify_Bedrock/BedrockCommand.h)
...
- **Файл:** [common.h](file://~/oss/ElementsProject_lightning/db/common.h)
...
- **Файл:** [MessageBuilder.hpp](file://~/oss/Chatterino_chatterino2/src/messages/MessageBuilder.hpp)
...
- **Файл:** [objectSampler.h](file://~/oss/DataDog_java-profiler/ddprof-lib/src/main/cpp/objectSampler.h)
...
- **Файл:** [interpreter.h](file://~/oss/Emute-Lab-Instruments_uSEQ/uSEQ/src/lisp/interpreter.h)
...
- **Oiiotool** (42) — [oiiotool.h](file://~/oss/AcademySoftwareFoundation_OpenImageIO/src/oiiotool/oiiotool.h) — CLI-тумблеры `ap.arg(...)`, легально комбинируются (`-v -n --no-clobber`).
- **SettingsCache** (76) — [cache_settings.h](file://~/oss/Cockatrice_Cockatrice/cockatrice/src/client/settings/cache_settings.h) — синглтон QSettings, у каждого bool своя пара get/set + Qt-signal. Конфиг, не FSM.
- **Catch2 ConfigData** (15) — [catch_config.hpp](file://~/oss/Catch2/src/catch2/catch_config.hpp) — **вендор**; `list*` подтверждённо не взаимоисключающие (аддитивные `if` в catch_list.cpp).
- **uSEQ ConfigData** (12) — [catch.hpp](file://~/oss/Emute-Lab-Instruments_uSEQ/test/catch.hpp) — **вендор** (тот же Catch2).
- **daemon** (15) — [connectd.h](file://~/oss/ElementsProject_lightning/connectd/connectd.h) — мешок `dev_*`/config-тумблеров; ЖЦ соединения уже в отдельных enum (`draining_state`).
- **Arguments** (11) — [arguments.h](file://~/oss/DataDog_java-profiler/ddprof-lib/src/main/cpp/arguments.h) — agent-опции, проверяются комбинациями `_record_allocations || _record_liveness`.
- **MessagePreferences** (11) — [MessageLayoutContext.hpp](file://~/oss/Chatterino_chatterino2/src/messages/layouts/MessageLayoutContext.hpp) — снимок `enable*`-чекбоксов, несколько highlight одновременно.
- **print_info_options** (10) — [imageio_pvt.h](file://~/oss/AcademySoftwareFoundation_OpenImageIO/src/include/imageio_pvt.h) — options-bag, аддитивные `if`.
- **GLOBAL** (7) — [Uhr.h](file://~/oss/ESPWortuhr_Multilayout-ESP-Wordclock/include/Uhr.h) — EEPROM-конфиг прошивки; смена формата сломала бы хранение.
- **GCodeChecker** (5) — [GCodeChecker.h](file://~/oss/BambuStudio/bbs_test_tools/bbs_gcode_checker/GCodeChecker.h) — парсер-флаги фич G-кода, часто истинны вместе.
- **TextureSystemImpl** (5) — [texture_pvt.h](file://~/oss/AcademySoftwareFoundation_OpenImageIO/src/libtexture/texture_pvt.h) — runtime-атрибуты через `attribute()`.
- **BedrockTester** (5) — [BedrockTester.h](file://~/oss/Expensify_Bedrock/test/lib/BedrockTester.h) — тест-харнес CLI-тумблеры.

### GUI-настройки (gnuplot)
- **QtGnuplotWidget** (7) — [QtGnuplotWidget.h](file://~/oss/AlexanderTaeschner_gnuplot/src/qtterminal/QtGnuplotWidget.h) — Q_PROPERTY + QSettings, ортогональны.
- **QtGnuplotWindow** (5) — [QtGnuplotWindow.h](file://~/oss/AlexanderTaeschner_gnuplot/src/qtterminal/QtGnuplotWindow.h) — зеркало настроек виджета.
- **wxtConfigDialog** (5) — [wxt_gui.h](file://~/oss/AlexanderTaeschner_gnuplot/src/wxterminal/wxt_gui.h) — чекбоксы диалога; многозначные опции уже сделаны `int`.

### `has_*`/`_can_*`/`_use_*`-флаги наличия и capability
- **cgltf_material** (15) — [cgltf.h](file://~/oss/BlazingRenderer_BRender/core/fmt/cgltf.h) — **вендор** (cgltf); `has_*` секции/extension glTF, ортогональны.
- **cgltf_node** (5) — [cgltf.h](file://~/oss/BlazingRenderer_BRender/core/fmt/cgltf.h) — **вендор**; `has_translation/rotation/scale/matrix` (частичный TRS допустим).
- **VMStructs** (8) — [vmStructs.h](file://~/oss/DataDog_java-profiler/ddprof-lib/src/main/cpp/hotspot/vmStructs.h) — `_has_*`/`_can_*` capability JVM, кэш обнаружения.
- **AddElementNotificationClientCommand** (6) — [NotificationCommands.hpp](file://~/oss/ENZYME-APD_tapir-archicad-automation/archicad-addon/Sources/NotificationCommands.hpp) — битовая маска подписок (3 оси × 2 уровня).
- **IteratorBase** (6) — [imagebuf.h](file://~/oss/AcademySoftwareFoundation_OpenImageIO/src/include/OpenImageIO/imagebuf.h) — `m_valid`/`m_exists` ортогональные предикаты (все 4 комбинации осмысленны), горячий цикл.
- **api_connector** (5) — [api_connector.hpp](file://~/oss/EVerest_EVerest/applications/pionix_chargebridge/include/charge_bridge/everest_api/api_connector.hpp) — три `*_enabled` под-API (битовая маска фич).

### Lazy-cache / dirty / validity флаги
- **vcs_VolPhase** (8) — [vcs_VolPhase.h](file://~/oss/Cantera_cantera/include/cantera/equil/vcs_VolPhase.h) — 6 `m_UpToDate*` (инвалидация по независимым осям) + 2 property-флага.
- **InterfaceKinetics** (6) — [InterfaceKinetics.h](file://~/oss/Cantera_cantera/include/cantera/kinetics/InterfaceKinetics.h) — cache (`m_ROP_ok`) + `m_has_*` + skip-опции; пары `if(!m_jac_skip_X && m_has_X)`.
- **value_information** (13) — [rapidjson.h](file://~/oss/CoolProp_CoolProp/include/CoolProp/detail/rapidjson.h) — зеркало предикатов типа JSON (перекрываются: istrue+isbool); вдобавок мёртвый код.
- **GraphBuilder** (6) — [graph_builder.h](file://~/oss/Emute-Lab-Instruments_uSEQ/uSEQ/src/signal_engine/graph_builder.h) — sticky-error + 2 static init-guard + режим-флаг.

### Конфиг-свойства / режимы (не взаимоисключающие)
- **Reaction** (7) — [Reaction.h](file://~/oss/Cantera_cantera/include/cantera/kinetics/Reaction.h) — свойства реакции из YAML + флаги сериализации.
- **Reactor** (6) — [Reactor.h](file://~/oss/Cantera_cantera/include/cantera/zeroD/Reactor.h) — 2 enable + 4 skip-опции якобиана (64 легальные комбинации).
- **ResidualHelmholtzGeneralizedExponential** (7) — [Helmholtz.h](file://~/oss/CoolProp_CoolProp/include/CoolProp/fluids/Helmholtz.h) — `*_in_u` «какие члены присутствуют» (подмножества), не одно состояние.
- **ProfiledThread** (5) — [thread.h](file://~/oss/DataDog_java-profiler/ddprof-lib/src/main/cpp/thread.h) — 3 флага разных подсистем; тип потока УЖЕ вынесен в `enum ThreadType` (подтверждает рубрику).
- **SQLite** (10) — [SQLite.h](file://~/oss/Expensify_Bedrock/sqlitecluster/SQLite.h) — ортогональные латчи/режимы конкурентного объекта, истинны в произвольных комбинациях.
- **image_config_t** (7) — [BlueSCSI_disk.h](file://~/oss/BlueSCSI_BlueSCSI-v2/src/BlueSCSI_disk.h) — смесь конфиг-опций, латчей и one-shot guard на SCSI-таргете.
- **NoteControlBar** (6) — [NoteControlBar.h](file://~/oss/203-Systems_MatrixOS/Applications/Note/NoteControlBar.h) — ЖЦ-режим уже в `enum NoteControlBarMode`; остаток — per-key латчи.
- **Context** (5) — [peglib.h](file://~/oss/Cockatrice_Cockatrice/libcockatrice_utility/libcockatrice/utility/peglib.h) — **вендор** (cpp-peglib); re-entrancy guard + const-конфиг.
- **Definition** (13) — [peglib.h](file://~/oss/Cockatrice_Cockatrice/libcockatrice_utility/libcockatrice/utility/peglib.h) — **вендор**; ортогональные свойства правила грамматики.
- **Arg** (6) — [Arg.h](file://~/oss/BelledonneCommunications_flexisip/src/tclap/Arg.h) — **вендор** (TCLAP); 4 свойства-спецификации + 2 рантайм-флага.

### Биты протокола / статуса
- **EV102** (10) — [messages.hpp](file://~/oss/EVerest_EVerest/lib/everest/ieee2030_1_1/include/ieee2030/common/messages/messages.hpp) — биты fault/status CHAdeMO (`fault & (1<<n)`), несколько одновременно. Кандидат на `std::bitset`, не enum.
- **Charger109** (6) — [messages.hpp](file://~/oss/EVerest_EVerest/lib/everest/ieee2030_1_1/include/ieee2030/common/messages/messages.hpp) — симметрично, биты статуса станции.
- **mosquitto** (16) — [mosquitto_internal.h](file://~/oss/AetherSDR/third_party/mosquitto/src/mosquitto_internal.h) — **вендор**; ЖЦ уже в `enum mosquitto_client_state`, остаток — опции/рабочие флаги.
...
- **format_spec_t** (5) — [format.h](file://~/oss/ETLCPP_etl/include/etl/format.h) — независимые format-флаги (`#`,`0`,`L`); взаимоисключающие align/sign УЖЕ в отдельных enum.

---

## ⚪ FP экстрактора — это не поля структуры (2)

- **bit_stream** (7) — [bit_stream.h](file://~/oss/ETLCPP_etl/include/etl/bit_stream.h) — посчитаны локальные `bool success`/`bool result` в инлайн-методах. Полей-bool у структуры нет.
- **CodeGenerator** (5) — [code_generate.h](file://~/oss/BoleynSu-Org_monorepo/legacy/BSL-AlgorithmW/src/code_generate.h) — имя `first` (локальный «первый-в-списке» разделитель) посчитано из 5 разных областей. Реальных bool-полей нет.
