...
| `FUNC.COGNITIVE_COMPLEXITY` | per-function Sonar Cognitive Complexity ≥ threshold (nesting-weighted; advisory, default run) | `function '<name>' cognitive complexity <N> exceeds threshold <T>` | `functionComplexity = 40` (`include/archcheck/config/config.h:47`) | `scan/local_complexity_absolute.cpp` (+`_metrics`; called in `cli/check_command.cpp` `checkViolations`) | `integration/scan/local_complexity_fixtures_test.cpp` | `local_complexity_absolute/` |
