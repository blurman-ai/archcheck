...
Phase 1 of v1 ships **three top-level keys** and **three rule types**. Everything else (`baseline`, `ignore`, `required`, fan-in thresholds, `auto_modules`, `protected`, per-rule `severity`) is phase 2 or later — out of scope for this document.
...
Exactly these three top-level keys: `version`, `modules`, `rules`. Anything else is a config error (exit code `2`).
...
| `thresholds:` block (CCD/fan-in/depth) | v1 phase 2 | Same file, new key   |
