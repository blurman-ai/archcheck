...
_2026-07-15 · phase: **v0.1.8 — public-launch prep (#163); v0.2 (dependency policy rules) after the announcement**_
...
**v0.1.8 released** (2026-07-15; v0.1.0 shipped 2026-06-25): the trusted graph/drift/diff
...
v0.1.7/v0.1.8 are launch-blocking hygiene found by auditing the demo showcase the
announcement links as receipts (#188): `TEST.1` fired on the whitespace-only demo case that
exists to prove the detector stays quiet, and the PR comment's gate status was unreadable —
first as a literal `:large_yellow_circle:` (not a real GitHub alias), then, after the icon
fix, as a tofu box for any reader without an emoji font. The report is now pure ASCII
(`PASS` / `ADVISORY` / `FAIL`), which is the only spelling with no font dependency. The demo
workflow pins `v0.1.8` as its floor.
