...
**P1.3 (header-impl gate)** → header-impl (6 FP) — **NOT IMPLEMENTED**
- Detection: pairs between .h and .cpp with >70% declarations
- Impact (projected, not achieved): ~4 FP. In the code, P1.3 was a no-op stub
  (`(void)candidates`) and **never removed any FP**; the stub has been removed. Full
  implementation is planned — see #070.
...
FP removed:
  ...
  ├─ idiom (P0.1+P1.2):    -30
  ├─ generated (P0.6):     -7
  ├─ header-impl (P1.3):    0   (projected -4; P1.3 was a no-op stub, removed — see #070)
  ├─ symmetric (P0.5):     -5
  └─ other:                -5
────────────────
143 TP ✅
...
| P1.3 | header-impl gate | classifier | 70% | 0 (projected -4) | ❌ no-op, removed (#070) |
