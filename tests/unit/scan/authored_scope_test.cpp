...
TEST_CASE("AuthoredScope: SWIG banner in content excludes an ordinary-named file (#179 follow-up)",
          "[scan][authored_scope]")
{
  // The case above is caught by the *_wrap.* PATH suffix. A SWIG amalgamation with an
  // ordinary basename (ezsnmp_netsnmp.cpp) has no such marker — only the CONTENT banner
  // (hasGeneratedBannerMarker) can exclude it through the shared gate. Same path, the
  // banner is the sole difference: banner => excluded, no banner => authored.
  const auto scope = AuthoredScope::changedFilesMode();
  REQUIRE(scope.excluded("src/objectmodel.cpp", "/* This file was automatically generated by SWIG */\nint code(){}"));
  REQUIRE_FALSE(scope.excluded("src/objectmodel.cpp", "int code(){}"));
}
...
TEST_CASE("AuthoredScope: generated/copied banner excluded; doesn't flip the ratio (#127)", "[scan][authored_scope]")
{
  // Only the two banner-carrying files are generated (Coco/R, @generated). They must
  // be excluded AND must not count toward the dominant-banner ratio, so the plain
  // authored files stay in (would NOT, if generated files voted).
  const std::string coco = "/* Compiler Generator Coco/R */\nint p;";
  const std::string gen = "// @generated by codegen\nint q;";
  std::vector<std::pair<std::string, std::string>> files = {
      {"src/a.cpp", "int a;"}, {"src/b.cpp", "int b;"}, {"Parser.h", coco}, {"gen.h", gen}};
  const auto scope = AuthoredScope::fromFiles(files);
  REQUIRE(scope.excluded("Parser.h", coco));
  REQUIRE(scope.excluded("gen.h", gen));
  REQUIRE_FALSE(scope.excluded("src/a.cpp", "int a;"));
}

TEST_CASE("AuthoredScope: Bison/Flex generated files with ordinary names are excluded", "[scan][authored_scope]")
{
  const std::string parser = "// A Bison parser, made by GNU Bison 3.8.2.\n"
                             "// Skeleton implementation for Bison LALR(1) parsers in C++\n";
  const std::string scanner = "/* A lexical scanner generated by flex */\n#define FLEX_SCANNER\n";
  std::vector<std::pair<std::string, std::string>> files = {
      {"src/parser.cpp", parser}, {"src/scanner.cpp", scanner}, {"src/hand_parser.cpp", "int parse();"}};
  const auto scope = AuthoredScope::fromFiles(files);

  REQUIRE(scope.excluded("src/parser.cpp", parser));
  REQUIRE(scope.excluded("src/scanner.cpp", scanner));
  REQUIRE_FALSE(scope.excluded("src/hand_parser.cpp", "int parse();"));
}
