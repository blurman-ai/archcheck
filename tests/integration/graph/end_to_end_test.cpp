...
BuildResult build_graph(const std::filesystem::path& root)
{
   ...
   for (std::size_t i = 0; i < files.size(); ++i)
   {
      ...
      {
         if (r.resolution == Resolution::Project) out.graph.add_edge(id_map[i], id_map[r.target]);
         else if (r.resolution == Resolution::External)   ++out.external;
         else if (r.resolution == Resolution::Unresolved) ++out.unresolved;
         else if (r.resolution == Resolution::Ambiguous)  ++out.ambiguous;
      }
   }
   return out;
}
...
TEST_CASE("fixture: minimal_dag — 3 nodes, 2 edges, acyclic", "[graph][fixtures]")
{
   ...
   REQUIRE(sccs.size() == 3);
   for (const auto& c : sccs) REQUIRE(c.size() == 1);
}
...
TEST_CASE("fixture: single_scc — 3 nodes форма one SCC", "[graph][fixtures]")
{
   ...
   const auto sccs = compute_scc(r.graph);
   std::size_t big = 0;
   for (const auto& c : sccs) if (c.size() >= 2) ++big;
   REQUIRE(big == 1);
}
