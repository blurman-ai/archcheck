...
static void collectCppFiles(const std::string &root, 
...
TEST_CASE("Duplication detector on all 4 projects with P0/P1 guard stats", 
          "[duplication][batch][integration]") {
    
    std::vector<std::string> projectPaths = {
        "/home/localadm/projects/cpparch/sandbox/drift_repos/BambuStudio",
    ...
    for (const auto &pr : results) {
        ...
        snprintf(buf, sizeof(buf), "%-16s | %5d | %7zu | %8zu | %8zu | %7.1f%%\n",
                pr.name.c_str(), pr.fileCount, pr.candidatesP0, 
                pr.pairsP0, pr.pairsP1, pr.filterRate);
        std::cout << buf;
    }
    ...
    snprintf(sumBuf, sizeof(sumBuf), "%-16s | %5zu | %7zu | %8zu | %8zu | %7.1f%%\n",
            "TOTAL", totalFiles, totalCandidatesP0, totalPairsP0, totalPairsP1, aggFilterRate);
    std::cout << sumBuf;
    ...
    REQUIRE(totalPairsP0 >= 0);
    REQUIRE(totalPairsP1 >= 0);
}
