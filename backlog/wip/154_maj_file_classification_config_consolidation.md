...
Committed: the header symbols (`kGeneratedSuffixes`, `kImplSuffixes`/`isInlineImplFile`, `kPchBasenames`/`isKnownPchBasename`) landed in **ac76ab6** (swept in with #151, shipped in v0.1.1); the consumer wiring — `sf9_no_cycles.cpp`/`lakos_god_headers.cpp` redirected to the shared predicates, local `kImplMarkers`/`kKnownPch` removed — in **5916b77**. NB: between those two commits the header symbols were briefly dead code (the #151 sweep-in shipped in v0.1.1 before the consumers were wired); harmless (inert inline), noted for the record.
