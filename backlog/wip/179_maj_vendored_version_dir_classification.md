...
## Follow-up implementation (2026-07-08) — SWIG amalgams fixed; base-lib handled as corpus override

After review, the "not done" block above was treated as part of #179 follow-up, not as a reason to
close the task on net-snmp alone.

**Giant-amalgam closed in product code.** The ezsnmp giant files are not an unknown size heuristic:
historical contents show the exact top banner `This file was automatically generated by SWIG`. The
shared generated-header classifier now recognizes that narrow SWIG banner, so suffix-free SWIG outputs
such as `ezsnmp_netsnmp.cpp` / `ezsnmp_sessionbase.cpp` drop from snapshot-based graph/clone/complexity
signals. No broad line-count exclusion was added. Targeted `ezsnmp` diff check on the SWIG-introducing
commit shows no `DRIFT.NEW_CLONE` entries for those files; SATD still reports markers from added SWIG
comments because SATD scans raw added lines outside `SourceSnapshot`, which is a separate advisory-path
gap and not part of the corpus clone count.

**base-lib — superseded by author decision 2026-07-08 (see [#173] "Decision"): excluded from the
corpus outright, not classified.** Provenance re-check showed the "multi-mount" framing was wrong —
the re-mounts are pure renames the move-guard already collapses to 0 clone-drift, so base-lib's 1733
events are real internal copy-paste, not noise; dropping it is a conservative under-count. The config
sketch below is kept only as the mechanism for a clean *rerun* (a per-repo path exclusion, since
`base-lib`/`baselib` is too generic for a global default):
...
rules: []
classification:
  extra_vendored_dirs: ["base-lib"]
```

Manual check on `mla-core` commit `b844e5196e25ec75176a738c5403a1219430c3d8`: with that config,
`--diff b844e5~1..b844e5 .` no longer prints `base-lib` clone findings; authored `core-os/...` clone
findings remain. This is the intended split: the embedded library is excluded, project code still reports.

**cross-mount status.** No product-wide exact-file dedup was added because it would break the existing
`DRIFT.NEW_CLONE` contract that adding a copied file is still a signal. Cross-mount noise should be
removed in the corpus aggregation/rerun layer together with the project-specific override above.

**Verification this round:**
- `cmake --build build/debug`
- `python3 -m py_compile experiments/agentic_weak_review/archcheck_run/clone_and_run.py experiments/agentic_weak_review/archcheck_run/backfill_percommit.py` (local ignored runner helpers)
- `./build/debug/tests/archcheck_tests "[scan][generated]"`
- `./build/debug/tests/archcheck_tests "[scan][vendor]"`
- `./build/debug/tests/archcheck_tests "[scan][newclone]"`
- `./build/debug/tests/archcheck_tests "[duplication][path-guards]"`
