...
- Задача перемещена в wip (2026-06-02)
- **token_normalizer.{h,cpp}**: lexer + selective-norm (keepCalls, case-labels, raw-string, dead-`#if 0` skip)
- **fragmenter.{h,cpp}**: function-scale fragment extraction with brace-balance heuristic
- **similarity.{h,cpp}**: weighted/plain Jaccard, line overlap, token-LCS, LCS ratio
- **clone_classifier.{h,cpp}**: DiffOp edit script + clone type classification (EXACT/RENAMED/LITERAL/MIXED/STRUCTURAL)
- Добавлены модули в src/CMakeLists.txt
- Unit-тесты token_normalizer (5 тестов, все зелёные)
- Сборка Debug: зелёная

## В работе

1. Porting remaining modules:
   - `clone_index.{h,cpp}` (inverted index + rare-token filtering)
   - `duplication_scanner.{h,cpp}` (main orchestration: scan→fragment→index→pair→score→classify)
   - Reporters (text + json output)
2. FP-гардлы из #070 (idiom stop-list, AND-gating, file-local IDF, header-skeleton suppression)
3. Фикстуры для каждого гардла
4. dogfood integration (archcheck на собственном коде)

## Следующие шаги

1. Ported 4/6 основных модулей (83% по структуре)
2. Продолжить с clone_index и duplication_scanner
3. Расширить юнит-тесты (similarity, fragmenter, classifier)
4. Коммитить по-модульно после каждого полного цикла (тест+компиляция)
