...
- [x] `--config` больше не создаёт ложного впечатления runtime-enforcement. *(Slice 1: help + config_format «Enforcement status»; Slice 2: подтверждено — runtime только thresholds.)*
- [x] Diagnostics contract (`line`/`column`) одинаков в коде и документах. *(Slice 1: docs → `file:line`; Slice 2: код уже `file:line`.)*
- [x] В shipped core не остаётся placeholder/no-op логики, выдаваемой за готовую feature. *(Slice 5a: no-op `phase12` удалён; `fp_corpus_eval` — internal QA вне user-facing surface.)*
- [x] `AGENTS.md` соответствует фактическому состоянию репозитория и актуальному style contract. *(Slice 3: полный rewrite.)*
- [x] `README`, `MVP`, `ROADMAP`, `product_vision`, `architecture-spec` не противоречат друг другу по текущему shipped surface. *(Slice 1/5b/6: diagnostics, --config, duplication-статус, CLI-форма выровнены.)*
- [x] Broken local markdown links, найденные в рамках этой темы, либо исправлены, либо переведены в честные historical notes. *(Slice 4: 26 исправлено, experiments де-линкован в historical notes.)*
- [x] Research/preview ветки больше не маскируются под product core без явной маркировки. *(Slice 5b: duplication → явный shipped advisory; experiments — local sandbox помечен.)*
- [x] После завершения задачи новый разработчик может за один проход понять:
      что shipped, что preview, что research, что future. *(достигнуто слоями 1-6.)*
...
Исправлено **26**; 1 — false-positive (C++ lambda-синтаксис вида `cb`-capture
в прозе #070 — выглядит как markdown-ссылка, но это код; оставлен).
...
Безопасная часть Slice 5: убрать placeholder-маскарад, исправить ложные claims.
Не зависит от preview-vs-product решения.

- **код:** удалён no-op `phase12HeaderImplGate` (call + def) из
  `src/scan/duplication/duplication_scanner.cpp`. Поведение не изменилось
  (был `(void)candidates; (void)allFragments;`), 344/344 тестов зелёные.
- **docs/CHANGELOG:** P1.3 заявлялся как работающий классификатор («-4 FP», «70%»,
  «✅»), хотя код был заглушкой. Исправлено в `duplication_fp_analysis.md` (3 места),
  `duplication_architecture.md`, `CHANGELOG.md` (10 фильтров → 9; P1-классификаторов
  4 → 3; P1.3 помечен как no-op-заглушка, удалён, планируется в #070).
...
Сквозная проверка цепочки «обещание → код → вывод → docs → backlog» по слоям 1-5b.
Все автоматические проверки чисты:

- `file:line:column` — 0 вхождений (код + все docs);
- `phase12`/no-op gate — 0;
- `--config` enforcement over-promise в README/MVP/spec — нет;
- duplication = один ответ везде (advisory, не gate) — stray preview-фрейминга нет;
- битые локальные ссылки — 0 реальных (1 false-positive: C++ lambda в #070);
- `archcheck check`/`init` subcommand-форма (CLI не имеет subcommand) — исправлена в
  `MVP.md` на flag-форму `archcheck --config`; `init` помечен v0.2 (not shipped).
  Оставшиеся `archcheck synthesize` — явно future (#010), не противоречат shipped.

**Acceptance criteria — статус:** все 8 закрыты (см. чеклист выше; placeholder-критерий
закрыт для shipped runtime-пути — `phase12` удалён, `fp_corpus_eval` это internal QA-tooling
