...
Хвосты разнесены по задачам (2026-06-12):

1. → **#112** (Haiku) — перегенерация секции A в `EXAMPLES_50.md`.
2. → **#115** — переклон 297 реп + полный прогон + нормированный agentic-разрез
   (решение «клонировать» принято).
3. → **#113** (Haiku) — Apache-баннер ≠ вендор при доминировании в проекте.
4. → **#114** (Haiku) — `-test`/`testutil` в тест-фильтр.
5. Грейс-период time-based (30 дней) — не выделен в задачу: крутить при
   полном прогоне #115, если NEW-шум на новорождённых модулях останется.
