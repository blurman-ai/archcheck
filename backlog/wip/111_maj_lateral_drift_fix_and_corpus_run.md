...
1. Перегенерить секцию A в `EXAMPLES_50.md` по LATERAL-событиям (план Фазы 3).
2. Нормированный agentic vs human разрез (repo fixed effects) — 12 событий мало,
   возможно ждёт расширения корпуса.
3. Решить судьбу 297 удалённых реп (переклон ~30–40 ГБ или зафиксировать 183).
4. Завести задачи в archcheck: (а) Apache-баннер ≠ вендор при доминировании в
   проекте; (б) `-test`/`testutil` в тест-фильтр.
5. Грейс-период: кандидат на time-based (30 дней) вместо 10 коммитов.
...
| Файл | Изменение |
|------|-----------|
| `experiments/ai_repo_run/generate_per_commit_graph_drift.py` | Fix A/B (если нужен) |
| `experiments/ai_repo_run/lateral_drift_scan.py` | Новый скрипт-прототип |
| `experiments/ai_repo_run/lateral_drift_new.csv` | Output |
| `docs/research/lateral_module_drift_corpus_run.md` | Отчёт прогона |
