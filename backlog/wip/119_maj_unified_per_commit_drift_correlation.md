...
## Следующие шаги (опционально, не блокируют вывод)

1. Развернуть `n_other` в lateral/coupling-колонки (сигналы в датасете, см. примечание в шапке) —
   усилит матрицу графовыми деталями.
2. Глубже agentic-разметку: «human» = floor по AI (IDE-автокомплит невидим) — оценить чувствительность
   вывода D к перемаркировке.
3. Если найдётся сильная связь при добавлении lateral — поштучно сверить.
