...
**Приоритет:** major
**Сложность:** large
**Related:** #056 (token-dup детектор), #070 (precision-фильтры), #054 (AI-repo dense-корпус), #053 (line-dup)
...
confirm-слой (#070)** — он и дал размеченные данные, т.е. работает; нужно встроить как
гейт. Ветка: `feat/060-checker-hardening` (не пушена), код-дифф = 15 строк (#061+#064).

## В работе

- Реализация confirm-слоя (#070) как стадии пайплайна — следующий шаг.
