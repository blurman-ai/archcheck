...
## Итог
**Статус:** completed (секция 1 — мёртвый код; commit be56245).
Секции 2–4 (дубли >5 строк, гигиена duplication_scanner, lizard-долг) отложены
по решению в теле задачи → носитель `backlog/new/108_min_post_audit_sweep_rest.md`.
Пост-скриптум: один пункт секции 2 (`toLowerCopy`-дубль) независимо переоткрыт
self-scan'ом в #107 — подтверждение, что детектор находит ровно этот класс долга.
