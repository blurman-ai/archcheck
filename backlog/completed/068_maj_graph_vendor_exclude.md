...
## ⚠️ Коррекция (2026-06-01, #069)

Заявленный ниже код (`kExcludedVendor` в `project_files.cpp`) **в репозитории
отсутствовал** — `git log -S"kExcludedVendor"` по всем веткам пуст, граф строился по
всему дереву включая вендор. Работа была потеряна / не закоммичена. Dir-вендор-эксклюд
**реально реализован и покрыт тестами в #069**, но в другой точке: не в `discoverFiles`
(там тест требует сурфейсить `third_party/`), а в `buildGraphForSource` (`filterVendored`
→ `pathHasVendoredDir`), список `kVendoredDirNames` в `include/archcheck/scan/file_classification.h`.
Описание ниже сохранено как исходный замысел; актуальная реализация — см. #069.
