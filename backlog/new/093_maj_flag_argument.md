...
1. Не изобретать новый источник файлов: для full scan брать `scan::FileSource` и `collectNonVendoredSources()` из [src/scan/project_files.cpp](/home/localadm/projects/cpparch/src/scan/project_files.cpp), потому что там уже централизованы vendor/test exclusions.
2. Вынести text-preprocess primitives из [src/scan/include_scanner.cpp](/home/localadm/projects/cpparch/src/scan/include_scanner.cpp) в новый shared helper `include/archcheck/scan/text_scan.h` + `src/scan/text_scan.cpp`:
...
5. Full-scan wiring делать в [src/main.cpp](/home/localadm/projects/cpparch/src/main.cpp) внутри `run_check()`:
...
6. Delta wiring делать не через `RegressionReport`, а рядом с `run_diff()`:
   file prefilter брать из `git::changedCppFiles()` в [src/git/git_state.cpp](/home/localadm/projects/cpparch/src/git/git_state.cpp),
...
7. Тестовую матрицу раскладывать так:
   ...
   repo-level diff scenarios — расширение [tests/integration/diff/git_diff_test.cpp](/home/localadm/projects/cpparch/tests/integration/diff/git_diff_test.cpp).
