...
/home/localadm/projects/cpparch/build/debug/src/archcheck --save-graph-baseline "/tmp/clean_${label}_base.json" "$repo/$sub" 2>&1 | tail -1
git clean -fdx "$sub" >/dev/null 2>&1
git checkout -f "$after" -- "$sub" 2>/dev/null
/home/localadm/projects/cpparch/build/debug/src/archcheck --drift-baseline "/tmp/clean_${label}_base.json" "$repo/$sub" > "/tmp/clean_${label}_drift.txt" 2>&1 || true
echo "=== $label ==="
