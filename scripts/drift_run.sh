...
/home/localadm/projects/cpparch/build/debug/src/archcheck --save-graph-baseline "/tmp/drift_${label}_base.json" "$repo/$sub" 2>&1 | tail -1

# After revision: clean, checkout, scan and report drift
git clean -fdx "$sub" >/dev/null
git checkout -f "$after" -- "$sub"
/home/localadm/projects/cpparch/build/debug/src/archcheck --drift-baseline "/tmp/drift_${label}_base.json" "$repo/$sub" > "/tmp/drift_${label}_drift.txt" 2>&1 || true
